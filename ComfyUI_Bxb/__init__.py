...
class SDClient:
    ...
    def __init__(self, local_port, subdomain):
        ...
        self.server_port = "3000" #"7000"
        self.token = "1406" #"my_secure_token"
...
async def tech_zhulu(request):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(
                ...
                json_data['r'] + '&techsid=we7sid-' + json_data['techsid'], json=json_data) as resp:
            if resp.status == 200 and resp.headers.get('Content-Type') == 'application/json':
                try:
                    other_api_data = await resp.json()
                    print(f"zhangtao tech_zhulu() session post: other_api_data:{other_api_data}")
                    return web.json_response(other_api_data)
                except aiohttp.ContentTypeError:
                    error_text = await resp.text()
                    print(f"zhangtao tech_zhulu() session error_text: error_text:{error_text}")
                    return web.Response(text=error_text, status=400)
            else:
                result = web.Response(status=resp.status, text=await resp.text())
                print(f"zhangtao tech_zhulu() session status: result:{result}")
                return result
